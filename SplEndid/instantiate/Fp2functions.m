...
intrinsic GetTonelliShanksConstants(p::RngIntElt) -> FldFin, FldFin, SeqEnum
{Get the Tonelli--Shanks constants needed for computing square roots in Fp2
...
intrinsic Expon(a::FldFinElt, n::RngIntElt, mulcount::RngIntElt) -> FldFinElt, RngIntElt
{Given a in Fp2 and integer n computes a^n and updates number of Fp multiplications
...
FldFinElt
  The value a^n
RngIntElt
  mulcount +:= number of muls performed in F_p
}
  if n ne 0 then
...
intrinsic TonelliShanksUpdate(x::FldFinElt, y::FldFinElt, constants::SeqEnum, mulcount::RngIntElt) -> FldFinElt, FldFinElt, RngIntElt
{Performs the update step in the Tonelli--Shanks algorithm
...
intrinsic InvSqrt(u::FldFinElt, v::FldFinElt, constants::SeqEnum, mulcount::RngIntElt) -> FldFinElt, FldFinElt, RngIntElt
{Merged inversion-and-square root computation (see Section 3.1) using Tonelli--Shanks
...
-------
  ...
  c0,c1,c2,c3,c4,c5,c6,c7,c8,c9 := Explode(constants);
...
intrinsic InvBaseSqrt(u::FldFinElt, v::FldFinElt, constants::SeqEnum, mulcount::RngIntElt) -> FldFinElt, FldFinElt, RngIntElt
{Special case of InvSqrt where v is in the base field GF(p)
...
intrinsic InvFp2(D::FldFinElt, mulcount::RngIntElt) -> FldFinElt, RngIntElt
{Computes inverse of D over Fp2 and updates the number of Fp multiplcations
...
intrinsic BatchMontgomeryInversion(L::SeqEnum, mulcount::RngIntElt) -> SeqEnum
{Batch Montgomery inversion for a list of elements of a finite field
...
------
SeqEnum
  [1/L[1],...,1/L[n]] using 3*(n-1) multiplications (in Fp2) and 1 inversion
}
  M:=[L[1]];
  for i:=2 to #L do
